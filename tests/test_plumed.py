...
class TestPlumedSelection:
    ...
    def test_formats_runs_compactly(self, indices: list[int], expected: str) -> None:
        assert plumed_selection(indices) == expected

    def test_converts_to_one_based_indexing(self) -> None:
        """PLUMED counts atoms from 1, ASE from 0."""
        assert plumed_selection([0]) == "1"

    def test_sorts_the_input(self) -> None:
...
class TestFindMolecules:
    def test_finds_a_single_molecule(self, water: Atoms) -> None:
        groups = find_molecules(water)

        assert len(groups) == 1
        assert sorted(groups[0]) == [0, 1, 2]

    def test_separates_two_distant_molecules(self) -> None:
        first = molecule("H2O")
        second = molecule("H2O")
    ...
    def test_groups_partition_every_atom(self) -> None:
    ...
    def test_isolated_atoms_are_their_own_groups(self) -> None:
        atoms = Atoms("H3", positions=[[0, 0, 0], [20, 0, 0], [40, 0, 0]])

        assert len(find_molecules(atoms)) == 3

    def test_output_feeds_plumed_selection(self, water: Atoms) -> None:
...
class TestPlumedMetadInput:
    def test_builds_the_expected_lines(self) -> None:
    ...
    def test_declares_ase_units_first(self) -> None:
    ...
    def test_units_can_be_turned_off(self) -> None:
        lines = plumed_metad_input(
    ...
    def test_biases_every_collective_variable(self) -> None:
        lines = plumed_metad_input(
    ...
    def test_one_sigma_covers_every_variable(self) -> None:
        lines = plumed_metad_input(
    ...
    def test_hills_default_matches_run_sum_hills(self) -> None:
    ...
    def test_plain_metadynamics_has_no_bias_factor(self) -> None:
        lines = plumed_metad_input(
            cvs=["d: DISTANCE ATOMS=1,2"], sigma=0.1, height=0.01, pace=10
        )
    ...
    def test_the_colvar_print_can_be_dropped(self) -> None:
        lines = plumed_metad_input(
    ...
    def test_extra_keywords_land_on_the_metad_line(self) -> None:
        lines = plumed_metad_input(
    ...
    def test_extra_lines_are_appended(self) -> None:
        lines = plumed_metad_input(
            cvs=["d: DISTANCE ATOMS=1,2"],
            sigma=0.1,
            height=0.01,
    ...
    def test_accepts_a_selection_from_plumed_selection(self) -> None:
    ...
    def test_rejects_a_sigma_per_variable_mismatch(self) -> None:
        with pytest.raises(ValueError, match="2 sigmas for 1"):
            plumed_metad_input(
                cvs=["d: DISTANCE ATOMS=1,2"], sigma=[0.1, 0.2], height=0.01, pace=10
            )

    def test_well_tempered_needs_a_temperature(self) -> None:
    ...
    def test_rejects_a_bias_factor_of_one_or_less(self) -> None:
...
class TestPlumedCalculator:
    ...
    def test_attaches_the_biased_calculator_for_the_block(
        self,
        dimer: Atoms,
        metad_lines: list[str],
    ) -> None:
    ...
    def test_puts_the_original_calculator_back(
        self,
        dimer: Atoms,
        metad_lines: list[str],
    ) -> None:
    ...
    def test_without_the_units_line_it_comes_back_in_nanometres(
        self,
        dimer: Atoms,
        tmp_path: Path,
    ) -> None:
    ...
    def test_the_bias_changes_the_forces(
        self,
        dimer: Atoms,
        metad_lines: list[str],
    ) -> None:
...
class TestSumHillsFiles:
    def _touch(self, tmp_path: Path, names: Sequence[str]) -> None:
        for name in names:
            (tmp_path / name).write_text("#! FIELDS d1 file.free\n")

    def test_collects_a_strided_series(self, tmp_path: Path) -> None:
    ...
    def test_orders_numerically_not_lexicographically(self, tmp_path: Path) -> None:
    ...
    def test_a_stem_outfile_gives_tidier_names(self, tmp_path: Path) -> None:
    ...
    def test_an_unstrided_run_has_no_series(self, tmp_path: Path) -> None:
    ...
    def test_looks_in_the_working_directory_by_default(self, tmp_path: Path) -> None:
