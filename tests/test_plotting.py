...
@pytest.fixture
def band(calc: EMT, water: Atoms) -> list[Atoms]:
...
class TestStyleHelpers:
    def test_n_plot_labels_the_current_axes(self) -> None:
        fig, ax = plt.subplots()
        plt.sca(ax)
    ...
    def test_ax_plot_labels_the_given_axes(self) -> None:
    ...
    def test_ax_plot_applies_the_font_sizes(self) -> None:
        fig, ax = plt.subplots()
...
class TestPlotImages:
    def test_one_panel_per_image(self, chain: list[Atoms]) -> None:
        fig, axes = plot_images(chain, show=False)

        assert len(axes) >= len(chain)

    def test_accepts_a_single_atoms_object(self, water: Atoms) -> None:
        fig, axes = plot_images(water, show=False)

        assert len(axes) == 1

    def test_titles_default_to_the_image_index(self, chain: list[Atoms]) -> None:
        fig, axes = plot_images(chain, show=False)
    ...
    def test_custom_titles_are_used(self, chain: list[Atoms]) -> None:
        titles = ["R", "a", "TS", "b", "P"]
    ...
    def test_rejects_a_title_count_mismatch(self, chain: list[Atoms]) -> None:
...
class TestGetEnergy:
    def test_uses_the_cached_energy(self, band: list[Atoms]) -> None:
    ...
    def test_falls_back_to_the_calculator(self, calc: EMT, water: Atoms) -> None:
        water.calc = None
...
class TestPlotNeb:
    def test_returns_a_figure_and_axes(self, band: list[Atoms], calc: EMT) -> None:
        fig, ax = plot_neb(band, calc, save=False, show=False)

        assert fig is ax.get_figure()

    def test_energies_are_referenced_to_the_lowest_image(
        self, band: list[Atoms], calc: EMT
    ) -> None:
        fig, ax = plot_neb(band, calc, smooth=False, save=False, show=False)

        assert ax.lines[0].get_ydata().min() == pytest.approx(0.0)

    def test_energies_are_converted_to_mev(self, band: list[Atoms], calc: EMT) -> None:
        fig, ax = plot_neb(band, calc, smooth=False, save=False, show=False)
    ...
    def test_x_axis_is_the_path_coordinate(self, band: list[Atoms], calc: EMT) -> None:
        fig, ax = plot_neb(band, calc, smooth=False, save=False, show=False)
    ...
    def test_smoothing_adds_interpolated_points(
        self, band: list[Atoms], calc: EMT
    ) -> None:
    ...
    def test_does_not_annotate_by_default(self, band: list[Atoms], calc: EMT) -> None:
        """Existing figures must not gain text they did not have."""
        fig, ax = plot_neb(band, calc, save=False, show=False)

        assert len(ax.texts) == 0

    def test_annotates_the_barrier_when_asked(
        self, band: list[Atoms], calc: EMT
    ) -> None:
    ...
    def test_the_annotated_barrier_matches_summarise_neb(
        self, band: list[Atoms], calc: EMT
    ) -> None:
    ...
    def test_draws_on_a_supplied_axes(self, band: list[Atoms], calc: EMT) -> None:
        fig, ax = plt.subplots()
    ...
    def test_save_writes_the_supplied_figure(
        self, band: list[Atoms], calc: EMT
    ) -> None:
    ...
    def test_label_is_applied(self, band: list[Atoms], calc: EMT) -> None:
        fig, ax = plot_neb(
    ...
    def test_axis_labels_are_set(self, band: list[Atoms], calc: EMT) -> None:
        fig, ax = plot_neb(band, calc, save=False, show=False)
...
class TestPlotTemperature:
    def test_returns_a_figure_and_axes(self, md_trajectory: Path) -> None:
        fig, ax = plot_temperature(md_trajectory)

        assert fig is ax.get_figure()

    def test_accepts_a_supplied_axes(self, md_trajectory: Path) -> None:
    ...
    def test_plots_one_line_per_trajectory(self, md_trajectory: Path) -> None:
        fig, ax = plot_temperature([md_trajectory, md_trajectory])

        assert len(ax.lines) == 2

    def test_labels_default_to_the_filename(self, md_trajectory: Path) -> None:
        fig, ax = plot_temperature(md_trajectory)

        assert ax.lines[0].get_label() == "md.traj"

    def test_custom_labels_are_used(self, md_trajectory: Path) -> None:
        fig, ax = plot_temperature(md_trajectory, labels=["300 K run"])

        assert ax.lines[0].get_label() == "300 K run"

    def test_x_axis_is_frames_without_a_timestep(self, md_trajectory: Path) -> None:
        fig, ax = plot_temperature(md_trajectory)

        assert ax.get_xlabel() == "Frame"
        assert ax.lines[0].get_xdata() == pytest.approx([0, 1, 2, 3, 4])

    def test_timestep_switches_the_x_axis_to_time(self, md_trajectory: Path) -> None:
    ...
    def test_y_axis_is_temperature(self, md_trajectory: Path) -> None:
        fig, ax = plot_temperature(md_trajectory)
...
class TestPlotTotalEnergy:
    ...
    def test_plots_the_trajectory_energies(self, md_trajectory: Path) -> None:
    ...
    def test_y_axis_is_total_energy(self, md_trajectory: Path) -> None:
        fig, ax = plot_total_energy(md_trajectory)
...
class TestFesHelpers:
    def test_expand_accepts_a_single_file(self, fes_file: Path) -> None:
        assert _expand_fes_files(fes_file) == [fes_file]

    def test_expand_finds_fes_files_under_a_directory(self, tmp_path: Path) -> None:
    ...
    def test_expand_rejects_a_directory_with_no_surfaces(self, tmp_path: Path) -> None:
    ...
    def test_labels_use_the_file_stem(self, tmp_path: Path) -> None:
        files = [tmp_path / "cold.dat", tmp_path / "hot.dat"]

        assert _fes_labels(files) == ["cold", "hot"]

    def test_labels_fall_back_to_the_run_directory(self, tmp_path: Path) -> None:
...
class TestPlotPlumed:
    def test_returns_a_figure_and_axes(self, fes_file: Path) -> None:
        fig, ax = plot_plumed(fes_file, save=False, show=False)

        assert fig is ax.get_figure()

    def test_plots_the_surface_in_mev(self, fes_file: Path) -> None:
...
class TestPlotPlumedMulti:
    ...
    def test_one_line_per_run(self, runs: Path) -> None:
        fig, ax = plot_plumed_multi(runs, save=False, show=False)

        assert len(ax.lines) == 2

    def test_labels_come_from_the_run_directories(self, runs: Path) -> None:
        fig, ax = plot_plumed_multi(runs, save=False, show=False)

        assert [line.get_label() for line in ax.lines] == ["run_a", "run_b"]

    def test_custom_labels_are_used(self, runs: Path) -> None:
    ...
    def test_mintozero_shifts_each_curve_independently(self, runs: Path) -> None:
        fig, ax = plot_plumed_multi(runs, mintozero=True, save=False, show=False)

        for line in ax.lines:
            assert line.get_ydata().min() == pytest.approx(0.0)

    def test_offsets_are_kept_without_mintozero(self, runs: Path) -> None:
    ...
    def test_runs_get_different_colours(self, runs: Path) -> None:
        fig, ax = plot_plumed_multi(runs, save=False, show=False)

        assert ax.lines[0].get_color() != ax.lines[1].get_color()

    def test_accepts_an_explicit_file_list(self, runs: Path) -> None:
    ...
    def test_save_writes_png_and_pdf(self, runs: Path) -> None:
...
class TestShowAtoms:
    def test_accepts_a_single_structure(self, water: Atoms) -> None:
        fig, ax = show_atoms(water, save=False, show=False)

        assert fig is not None and ax is not None

    def test_overlays_a_whole_band_on_one_axes(self, band: list[Atoms]) -> None:
    ...
    def test_draws_on_a_given_axes(self, water: Atoms) -> None:
...
class TestPlotIrc:
    def test_returns_a_figure_and_axes(self, band: list[Atoms]) -> None:
    ...
    def test_plots_in_mev_above_the_minimum(self, band: list[Atoms]) -> None:
    ...
    def test_composes_onto_an_existing_axes(self, band: list[Atoms]) -> None:
        fig, ax = plt.subplots()
    ...
    def test_saves_both_formats(self, band: list[Atoms], tmp_path: Path) -> None:
        plot_irc(band, save=True, show=False, filename="path")

        assert (tmp_path / "path.png").exists()
        assert (tmp_path / "path.pdf").exists()

    def test_labels_the_axes_in_mev(self, band: list[Atoms]) -> None:
