...
class TestOpesFesCommand:
    ...
    def test_it_names_the_state_and_the_output(self):
    ...
    def test_the_grid_is_left_out_when_no_bounds_are_given(self):
        cmd = _opes_fes_command()

        assert "--min" not in cmd
        assert "--max" not in cmd

    def test_bounds_and_bins_are_passed_through(self):
    ...
    def test_kt_is_formatted_rather_than_dumped_at_full_precision(self):
        cmd = _opes_fes_command(kt=2.494338785445972)

        assert cmd[cmd.index("--kt") + 1] == "2.49434"

    def test_half_a_grid_is_rejected(self):
