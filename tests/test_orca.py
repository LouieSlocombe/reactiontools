...
def test_orca_calc_preset_defaults_solvation_and_dispersion_keywords():
    ...
    default = orca_calc_preset(orca_path="/nonexistent/orca", f_solv=True, f_disp=True)
...
def test_orca_calc_preset_builds_a_qmmm_region():
    calc = orca_calc_preset(
        orca_path="/nonexistent/orca", calc_type="QM/XTB2", atom_list="0:5"
    )
...
class TestPresets:
    ...
    def test_dft_cheap_is_blyp_in_the_gas_phase(self):
        calc = orca_calc_preset(orca_path="/nonexistent/orca", **orca_preset_dft_cheap)
    ...
    def test_dft_gold_is_b3lyp_with_d4_in_water(self):
        calc = orca_calc_preset(orca_path="/nonexistent/orca", **orca_preset_dft_gold)
    ...
    def test_xtb_names_the_method_and_takes_no_basis(self):
        calc = orca_calc_preset(orca_path="/nonexistent/orca", **orca_preset_xtb)
    ...
    def test_mp2_gold_uses_the_dlpno_approximation_and_an_aux_basis(self):
        calc = orca_calc_preset(orca_path="/nonexistent/orca", **orca_preset_mp2_gold)
    ...
    def test_ccsd_gold_is_canonical_rather_than_dlpno(self):
        ...
        calc = orca_calc_preset(orca_path="/nonexistent/orca", **orca_preset_ccsd_gold)
    ...
    def test_a_keyword_after_the_splat_overrides_the_preset(self):
        calc = orca_calc_preset(
            orca_path="/nonexistent/orca", **orca_preset_dft_cheap, n_procs=8
        )
